                ...
                <div class="flex flex-row bg-white rounded shadow p-3 mb-3">
                    ...
                    <div class="w-2/12 flex flex-col items-end justify-center">
                        <form method="post">
                            ...
                            <button @if($crypto->checkRating(Auth::id(), $crypto->id) == 1) class="w-24 h-8 mb-1 bg-green-500 text-white px-3 rounded" @elseif($crypto->checkRating(Auth::id(), $crypto->id) == 0 || $crypto->checkRating(Auth::id(), $crypto->id) == null) class="w-24 h-8 mb-1 bg-white border-solid border-2 border-green-500 text-green-500 px-3 rounded" @endif type="submit">Gem {{$crypto->RatingCount->gem_count ?? '0'}}</button>
